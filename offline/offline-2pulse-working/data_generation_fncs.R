  ...
  if(length(radiation_day)>0){
    for(pd1_idx in 1:num_pd1){
      ...
      for(start_time in start_times){
        pd1time = min(start_time+duration, totaltime)
        ll = length(start_time:pd1time)
      pd1_mat[(start_time):(pd1time),pd1_idx] = pd1_mat[(start_time):(pd1time),pd1_idx]+ seq(1,0, length.out = duration+1)[1:ll]
      }
    }
  }
