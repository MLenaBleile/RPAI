setwd("~/Documents/Dissertation/RPAI/online/online-3pulse-2param-v2")
source("data_generation_fncs v2.R")
source("env_fncs v2.R")
...
for(mouse in 1:100){
  ...
  one.random = sample(7:17,2, replace=T)
...
adj.val=2
plot(density(-agent.outcome.vec+ref.outcome.mat[,1], adjust=adj.val), col="red", xlab="final ltv reference-final ltv agent",main="3 pulse performance", xlim=c(-.4,.7), ylim=c(0,10))
...
for(ref.idx in 1:ncol(ref.outcome.mat)){
  ...
  effect.sizes[ref.idx] = mean(delta)/sd(delta)
  print(effect.sizes[ref.idx])
