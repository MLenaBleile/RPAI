...
make_parameter_mat = function(num_mice){
  ...
  #parameter_mat[,'BT0'] = truncnorm(num_mice, loc=BT0,scale=.01, lwr=0, upr=1)
  #parameter_mat[,"lambda"] = log(seq(1,exp(1), length.out=num_mice))
  parameter_mat[,"rho"] = truncnorm(num_mice, loc = rho, scale=.00009,upr=3, lwr=0)
  #parameter_mat[,"mu"] = truncnorm(num_mice, loc = mu, scale=.001,upr=1, lwr=0)
