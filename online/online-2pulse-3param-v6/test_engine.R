...
delta.diff.random = abs(optimal_actions-sample(potential_actions+waitime_vec[1], num_mice, replace=T)) - deltamodel
effect.sizes[4] = mean(delta.diff.random)
lines(density(abs(optimal_actions-sample(potential_actions+waitime_vec[1], num_mice, replace=T)) - deltamodel), col="purple")
