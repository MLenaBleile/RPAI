...
loss.weights=c(1,0)
names(loss.weights) = c("effects",'raw')
...
param_names=c("rho1","prop.1","prop.2", "beta0","beta1","gg")
...
random_params=c("rho1","gg","beta0","beta1")
...
bounds = matrix(nrow=10, ncol=2)
colnames(bounds) = c('upper','lower')
rownames(bounds) = c('omega1','omega2','alpha','beta','alpha1','phi','tau','rho', 'prop.1','prop.2')
bounds[,'lower'] = c(0.02,0.01,0,0,0,.5,5,0,0,0)
bounds[,'upper'] = c(.03,.02,.1,.1,.1,1,10,3,1.5,15)
...
for(mouse in 1:test_num){
  ...
  inc.pair = generate_one(radiation_days = c(), parameter_vec = true.param.vec, maxtime=inc_days)
  ...
  if(mouse>2){
    ...
    if(recompute_marginals){
      overall.params= optimize.model(pair.set=all.pairs[1:(mouse-1),,], param_names = param_names,bounds=bounds, mod.to.fit=mod.to.fit,fixed_param_vec=c(), loss.weights=loss.weights)
      overall.estimate.mat[mouse,]= overall.params
  ...
  #reestimated.par = optimize.model(pair.set=inc.pair, param_names = random_params,mod.to.fit=mod.to.fit,fixed_param_vec=fixed_param_vec,bounds=bounds, loss.weights=loss.weights)
  #est.par.mat[mouse,random_params] = estimated.par[random_params]
  predictive.params[random_params] = estimated.par[random_params]
  #*weight.factor.random
  #+colMeans(est.par.mat[,random_params], na.rm=T)*(1-weight.factor.random)
  ...
  selected_actions[mouse,] = one.action
  one.agent.treated.sequence = generate_one(cumsum(one.action)+15, parameter_vec=true.param.vec, maxtime=maxtime)
