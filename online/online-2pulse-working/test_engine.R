...
for(mouse in 1:test_num){
  ...
  one.reference = generate_one_counterfactualset(minibatch_parameters[mouse,],total_days = maxtime, wait_time = 0, potential_actions = potential_actions)
  ...
  for(refday.idx in 1:length(reference_days)){
    ...
    reference.outcomes[mouse, refday.idx]=log(one.reference[reference_day-wait_time,maxtime])
  }
