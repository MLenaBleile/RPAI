...
test_mice=tail(train_num+1:adapt_num, test_num)
...
reference_days = c(potential_actions)
optim_actions=c()
...
prop.betters = rep(NA, length(references))
prop.asgoodas=rep(NA, length(references))
for(refday.idx in 1:ncol(ref.outcome.mat)){
  ...
  prop.betters[refday.idx] = sum(loss>0)/sum(loss!=0)
  prop.asgoodas[refday.idx] = sum(loss>=0)/length(loss)
}
...
result = data.frame(means=loss.means, medians=loss.medians, midpoints=(loss.maxes-abs(loss.mins))/2, prop.better=prop.betters, prop.asgoodas=prop.asgoodas)
