setwd("~/RPAI/online/online-general-nn- sumexp")
...
train_num=1000/num_free_pulses
adapt_num=50/num_free_pulses
test_num = 50
...
one.pair= one_batch[1,1:inc_days,]
...
in.data = all.pca.data
actions = as.numeric(t(action_mat[1:train_num,]))
#in.data$act = all.pca.data[,'action'][train_idx]
ltv.vec=rep(one_batch[1:train_num,total_time,'ltv'], num_free_pulses)
in.data$ltv=ltv.vec

fit2 = neuralnet::neuralnet(ltv~(.), data=in.data[train_idx,], stepmax=10000,thresh=3,lifesign="full",hidden = c(6,4))
...
for(one.mouse in 1:adapt_num+train_num){
  ...
  one.in.df$action=potential_actions
  ...
  #in.data$action = all.pca.data[,'action']
  ...
  fit2 = neuralnet::neuralnet(ltv~., data=rbind(in.data, all.cf.preds), startweights = fit2$weights,thresh=2,stepmax=10000,hidden=c(6,4))
  cat("finished mouse",one.mouse,"\n")
